"""Backtest Hypothesis H2 (XAUUSD H4, re-specified) through canonical runner + MC.

Honest, no tuning. Runs H2_STRUCTURE_PULLBACK_H4 per H2_HYPOTHESIS.md. Reports
IS/OOS metrics + Monte Carlo per RESEARCH_PROTOCOL. New helper; no module modified.
"""
...
from edgelab.strategy.structure_pullback_h4_h2 import StructurePullbackH4H2Strategy
...
def main():
    ...
    print("\n=== H2: Structure Pullback (XAUUSD H4, re-specified) ===")
    is_res = run_canonical_backtest(is_df, StructurePullbackH4H2Strategy(),
    ...
    oos_res = run_canonical_backtest(oos_df, StructurePullbackH4H2Strategy(),
                                      initial_equity=10000.0, symbol=SYMBOL,
                                      risk_per_trade=RISK, spread_pips=SPREAD,
                                      slippage_pips=SLIPPAGE, session_windows=[[8, 0, 11, 0]])
    report(f"Out-of-sample ({oos_df.index[0].date()}..{oos_df.index[-1].date()})", oos_res.metrics, oos_res.trades)

    full = run_canonical_backtest(df, StructurePullbackH4H2Strategy(),
                                  initial_equity=10000.0, symbol=SYMBOL,
                                  risk_per_trade=RISK, spread_pips=SPREAD,
                                  slippage_pips=SLIPPAGE, session_windows=[[8, 0, 11, 0]])
    ...
    if not oos_pass:
        print("  => Per RESEARCH_PROTOCOL, H2 is RETIRED (or revisited with new evidence).")
